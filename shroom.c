...
void	make_zoom7(double x, double y, t_data *data)
{
	double			converted_i;
	double			converted_r;
...
void	make_dezoom7(double x, double y, t_data *data)
{
	double			converted_i;
	double			converted_r;
...
int		my_mouse9(int button, int x, int y, void *data)
{
	...
	if (button == 5)
		make_zoom7(x, y, data);
	if (button == 4)
		make_dezoom7(x, y, data);
	str = s->img_str;
	callcuda_shroom(s, x, y);
...
int		shroom(void)
{
	...
	callcuda_shroom(data, (LEN / 2), (LEN / 2));
